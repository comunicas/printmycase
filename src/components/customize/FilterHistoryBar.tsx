import { useEffect, useRef, useState } from "react";
import { Eye, Undo2, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import type { FilterHistoryEntry } from "@/lib/customize-types";
interface FilterHistoryBarProps {
  filterHistory: FilterHistoryEntry[];
  onCompareStart: () => void;
  onCompareEnd: () => void;
  onUndoLastFilter: () => void;
  onRemoveFilter: () => void;
  disabled?: boolean;
}

const FilterHistoryBar = ({
  filterHistory, onCompareStart, onCompareEnd, onUndoLastFilter, onRemoveFilter, disabled,
}: FilterHistoryBarProps) => {
  const [isHighlighting, setIsHighlighting] = useState(false);
  const prevLengthRef = useRef(filterHistory.length);

  useEffect(() => {
    if (filterHistory.length > prevLengthRef.current) {
      setIsHighlighting(true);
      const t = setTimeout(() => setIsHighlighting(false), 600);
      return () => clearTimeout(t);
    }
    prevLengthRef.current = filterHistory.length;
  }, [filterHistory.length]);

  const hasFilters = filterHistory.length > 0;

  return (
    <div className={`lg:hidden overflow-hidden transition-all duration-300 ${
      hasFilters ? "max-h-[200px] opacity-100" : "max-h-0 opacity-0"
    }`}>
      <div className={`bg-background border-t border-border px-3 py-2 space-y-1.5 transition-all duration-300 ${isHighlighting ? "animate-fade-in ring-2 ring-primary/30 rounded-t-lg" : ""} ${disabled ? "opacity-50 pointer-events-none" : ""}`}>
        <div className="flex items-center gap-1.5 overflow-x-auto flex-nowrap scrollbar-hide">
          <span className="text-[10px] text-muted-foreground font-medium whitespace-nowrap">
            {filterHistory.length} filtro{filterHistory.length > 1 ? "s" : ""}:
          </span>
          {filterHistory.map((entry, i) => (
            <span
              key={`${entry.filterId}-${i}`}
              className="inline-flex items-center gap-0.5 bg-primary/10 text-primary text-[10px] font-medium px-1.5 py-0.5 rounded-full whitespace-nowrap flex-shrink-0"
            >
              {i + 1}. {entry.filterName || "Filtro"}
            </span>
          ))}
        </div>
        <div className="flex gap-1.5">
          <Button
            variant="outline" size="sm"
            className="h-7 px-2 text-[11px] gap-1"
            disabled={disabled}
            onPointerDown={disabled ? undefined : onCompareStart}
            onPointerUp={disabled ? undefined : onCompareEnd}
            onPointerLeave={disabled ? undefined : onCompareEnd}
          >
            <Eye className="w-3 h-3" />
            Comparar
          </Button>
          {filterHistory.length > 1 && (
            <Button
              variant="outline" size="sm"
              className="h-7 px-2 text-[11px] gap-1"
              disabled={disabled}
              onClick={onUndoLastFilter}
            >
              <Undo2 className="w-3 h-3" />
              Desfazer
            </Button>
          )}
          <Button
            variant="outline" size="sm"
            className="h-7 px-2 text-[11px] gap-1 text-destructive hover:text-destructive"
            disabled={disabled}
            onClick={onRemoveFilter}
          >
            <X className="w-3 h-3" />
            Remover
          </Button>
        </div>
      </div>
    </div>
  );
};

export default FilterHistoryBar;
