...
const AiFiltersManager = () => {
  ...
  const openEdit = (filter: AiFilter) => {
    setEditing(filter); setName(filter.name); setPrompt(filter.prompt); setModelUrl(filter.model_url || MODEL_OPTIONS[0].value); setStyleImageUrl(filter.style_image_url || ""); setSendStyleImage(filter.send_style_image ?? true); setCategoryId(filter.category_id || ""); setDialogOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      const { error } = await supabase
        .from("ai_filters")
        .update({ name: name.trim(), prompt: noPromptNeeded ? "auto" : prompt.trim(), model_url: modelUrl, style_image_url: styleImageUrl || null, send_style_image: sendStyleImage, category_id: categoryId || null })
        .eq("id", editing.id);
      ...
      const { error } = await supabase
        .from("ai_filters")
        .insert({ name: name.trim(), prompt: noPromptNeeded ? "auto" : prompt.trim(), model_url: modelUrl, style_image_url: styleImageUrl || null, send_style_image: sendStyleImage, sort_order: maxOrder + 1, category_id: categoryId || null });
